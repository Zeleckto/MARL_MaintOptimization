...
class ManufacturingEnv(AECEnv if PETTINGZOO_AVAILABLE else object):
    ...
    def _resolve_physics(self) -> None:
        ...
        for old, new in zip(old_machine_states, self.machine_states):
            if old.status != MachineStatus.PM and new.status == MachineStatus.PM:
                self._episode_pm += 1
            if old.status != MachineStatus.CM and new.status == MachineStatus.CM:
                self._episode_cm += 1
