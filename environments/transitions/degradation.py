...
class DegradationEngine:
    ...
    def tick(
        ...
        if is_operating and state.status == MachineStatus.OP:
            ...
            # Use effective_age = virtual_age + time_since_maint
            # virtual_age alone is 0 until first repair (Kijima model);
            # time_since_maint accumulates the operating hours since last repair.
            effective_age = state.virtual_age + state.time_since_maint
            failed = sample_weibull_failure(
                virtual_age=effective_age,
                beta=state.beta,
                eta=state.eta,
                dt=self.dt,
                rng=rng,
            ...
            # Idle: no operating time accumulates.
            # Health is usage-based Weibull over operating hours only.
            pass
    ...
    def _apply_repair(self, state: MachineState, rng: np.random.Generator) -> MachineState:
        ...
        # Health recomputed from Weibull in _recompute_derived.
        # After repair: time_since_maint=0 -> effective_age drops -> health restores.
        # Kijima q controls how much age is retained. No manual h_restore needed.
    ...
    def _recompute_derived(self, state: MachineState) -> MachineState:
        ...
        # H(t) = 100 * exp(-(effective_age/eta)^beta)  [design doc v2]
        # H=80% -> 80% survival, 20% cumulative failure probability
        state.health = 100.0 * float(
            np.exp(-((effective_age / max(state.eta, 1e-6)) ** state.beta))
        )
