...
class Operation:
    ...
    def to_feature_vector(
        ...
        Features:
            ...
            [1]  min processing time across eligible machines (normalised by 60h)
            [2]  avg processing time across eligible machines (normalised by 60h)
        ...
        if self.nominal_proc_times:
            ...
            min_proc = min(proc_times) / 60.0
            avg_proc = sum(proc_times) / len(proc_times) / 60.0
        else:
        ...
        # Remaining time
        remaining_norm = self.remaining_time * dt_hours / 60.0
...
class JobDynamicsEngine:
    ...
    def __init__(self, config: dict):
        ...
        self.n_ops_max    = config.get("jobs", {}).get("n_ops_per_job_max", 6)
        ...
        self.t_max_train  = config.get("episode", {}).get("t_max_train", 150)
        # Processing time range (design doc Section 2: [16,56]h = [2,7] shifts)
        self.proc_time_min = config.get("jobs", {}).get("proc_time_min_hours", 16.0)
        self.proc_time_max = config.get("jobs", {}).get("proc_time_max_hours", 56.0)
        # Op-type compatibility: op_type_id -> eligible machine_ids (42.9% avg)
        default_compat = {0:[0,1,2], 1:[0,1,3], 2:[2,3,4],
                          3:[0,1], 4:[2,3], 5:[1], 6:[4]}
        raw = config.get("jobs", {}).get("op_type_compatibility", default_compat)
        self.op_type_compat = {int(k): list(v) for k, v in raw.items()}
        self.n_op_types = len(self.op_type_compat)
    ...
    def _generate_operations(
        ...
        for k in range(n_ops):
            # Assign op type from compatibility matrix (42.9% avg eligibility)
            op_type = int(rng.integers(0, self.n_op_types))
            eligible = list(self.op_type_compat[op_type])
            if not eligible:
                eligible = [int(rng.integers(0, self.n_machines))]

            # Proc time per eligible machine: Uniform [16,56]h = [2,7] shifts
            # mean=4.5 shifts -> utilisation 101.5%% (design doc Section 2)
            nominal_proc_times = {
                m: float(rng.uniform(self.proc_time_min, self.proc_time_max))
                for m in eligible
            }
