...
def compute_scheduling_reward(
    ...
    w_slack       = weights.get("w_slack", 0.1)
    ...
    # ── Slack signal (design doc §6.5) ─────────────────────────────────────
    # slack_j = due_date - current_time - remaining_proc_time
    # Counts jobs still on track (positive slack). Fires every step.
    # Immediate feedback: assigning a slow/degraded machine reduces slack next step.
    n_active = 0
    n_on_track = 0
    for job in jobs:
        if job.completion_time is not None:
        ...
        for op in job.operations:
            ...
            if op.status == OpStatus.IN_PROGRESS:
                # Estimate remaining work for this job
                remaining = getattr(op, "remaining_time", 0.0)
                slack = job.due_date - current_step - remaining
                n_active += 1
                if slack > 0:
                    n_on_track += 1
                break  # one in-progress op per job for slack check
    J_total = max(len(jobs), 1)
    slack_bonus = w_slack * n_on_track / J_total

    r2 = tard_penalty + make_penalty + comp_bonus + health_bonus + slack_bonus + lam * shared_reward
    return float(r2)